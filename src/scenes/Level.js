...
class Level extends Phaser.Scene {
	...
	editorCreate() {
		...
		// text_life
		const text_life = this.add.text(906, 100, "", {});
		...
		// timer_text
		const timer_text = this.add.text(566, 378, "", {});
	...
	create() {
		...
		for (let i = 0; i < this.oGameManager.totalPlatfrom; i++) {
	...
	addPlatform = (isFirstPlatform) => {
		...
		platform.assignedVelocityX = isFirstPlatform ? 0 : this.randomValue(this.oGameManager.platformHorizontalSpeedRange) * Phaser.Math.RND.sign();
		if (Math.random() < 0.3) {
	...
	destroyPlatform = () => {
		...
		if (this.canDestroy) {
			...
			closestPlatform.y = furthestPlatform.gameObject.y + this.randomValue(this.oGameManager.platformVerticalDistanceRange);
			closestPlatform.assignedVelocityX = this.randomValue(this.oGameManager.platformHorizontalSpeedRange) * Phaser.Math.RND.sign();
			closestPlatform.x = this.gameWidth / 2;
			closestPlatform.displayWidth = this.randomValue(this.oGameManager.platformLengthRange);
		}
		this.time2 = 6; // Reset the timer to 5 seconds
		this.timedEvent.paused = false; // Resume the timer
	}
	positionPlatform(platform) {
		platform.y = this.getLowestPlatform() + this.randomValue(this.oGameManager.platformVerticalDistanceRange);
		platform.x = this.gameWidth / 2;
		platform.setSize(275, 85);
		platform.displayWidth = this.randomValue(this.oGameManager.platformLengthRange);
	}
	getLowestPlatform() {
	...
	collectCarrot(hero, platform) {
		...
		if (platform.carrot) {
			...
			if (platform.texture.key == "p_1") {
				...
				console.log(this.collectedCarrot);
	...
	updateLevel = () => {
		currentLevel++;
		console.log("UpdateLevel", currentLevel);
		this.oGameManager.updatePlatformSpeedRange()
		// this.scene.start("Level");
	}
